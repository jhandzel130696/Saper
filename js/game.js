...
class Game extends UI{
    ...
    #newGame(rows=this.#config.easy.rows,cols=this.#config.easy.cols,mines=this.#config.easy.mines) {
        ...
        this.#revealMines();
        this.#modal.infoText= this.#timer.numberOfSeconds<this.#timer.maxNumberOfSeconds ? `You won, it took you ${this.#timer.numberOfSeconds} seconds` : `you won congratulations`
...
const colIndex = parseInt(target.getAttribute('data-x'),10);
    ...
    #setCellValue(cell){
        ...
        if(!cell.value){
            ...
            for ( let rowIndex = Math.max(cell.y-1,0); rowIndex<=Math.min(cell.y +1, this.#numberOfRows-1);rowIndex++ ){
                for(let colIndex= Math.max(cell.x-1,0);colIndex<=Math.min(cell.x +1, this.#numberOfCols-1); colIndex++){
                    const emptyCell=this.#cells[rowIndex][colIndex];
                   if(!emptyCell.isReveal){
            this.#clickCell(emptyCell);
                    }
                }
            }
        }
    ...
    #generateCells(){
        ...
        for( let row =0; row<this.#numberOfRows; row++){
            ...
            for(let col = 0; col< this.#numberOfCols; col++){
                ...
                //console.log('komorki',this.#cells);
    ...
    #renderBoard(){
        ...
        //console.log('tablica przed flat',this.#cells)
        //console.log('tablica po flat',this.#cells.flat())
